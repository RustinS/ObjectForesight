...
# Enable TF32 for spconv (faster fp32 on Ampere+ GPUs)
# Can be disabled via environment variable: SPCONV_DISABLE_TF32=1
import os

if os.environ.get("SPCONV_DISABLE_TF32", "0") != "1":
    spconv.constants.SPCONV_ALLOW_TF32 = True

_PTV3_ASSERT_INDICES = os.environ.get("PTV3_ASSERT_INDICES", "0") == "1"
_PTV3_ASSERT_BACKWARD = os.environ.get("PTV3_ASSERT_BACKWARD", "0") == "1"
...
def _assert_in_bounds(name: str, idx: torch.Tensor, *, size: int, extra: str = "") -> None:
    if idx.numel() == 0:
        return
    idx_min = int(idx.min().item())
    idx_max = int(idx.max().item())
    if idx_min < 0 or idx_max >= int(size):
        msg = f"PTV3 index out of bounds: {name} min={idx_min} max={idx_max} size={int(size)}"
        if extra:
            msg = f"{msg} • {extra}"
        raise RuntimeError(msg)


class _SafeIndexSelect(torch.autograd.Function):
    """Fancy indexing (src[idx]) with backward index validation.

    Replaces ``src[idx]`` and checks that ``idx`` values are in [0, src.shape[0])
    during BOTH forward and backward, raising a clear Python error instead of a
    CUDA device-side assert.
    """

    @staticmethod
    def forward(ctx, src: torch.Tensor, idx: torch.Tensor, name: str) -> torch.Tensor:
        _assert_in_bounds(f"{name}:fwd", idx, size=src.shape[0])
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        # Validate indices are still in-bounds before scatter
        _assert_in_bounds(
            f"{name}:bwd",
            idx,
            size=src_shape[0],
            extra=f"src_shape={list(src_shape)} idx_shape={list(idx.shape)} grad_shape={list(grad_output.shape)}",
        )

        grad_src = torch.zeros(src_shape, dtype=grad_output.dtype, device=grad_output.device)
        # Expand idx to match grad_output columns
        idx_expanded = idx.unsqueeze(-1).expand_as(grad_output)
        grad_src.scatter_add_(0, idx_expanded, grad_output)
        return grad_src, None, None


def _safe_index(src: torch.Tensor, idx: torch.Tensor, name: str) -> torch.Tensor:
    """Index ``src[idx]`` with backward index validation when PTV3_ASSERT_BACKWARD=1."""
    if _PTV3_ASSERT_BACKWARD:
        return _SafeIndexSelect.apply(src, idx, name)
    return src[idx]
...
class PDNorm(PointModule):
    ...
    def forward(self, point):
        ...
        if self.adaptive:
            ...
            shift_scale = torch.repeat_interleave(shift_scale, offset2bincount(point.offset), dim=0)
...
class SerializedAttention(PointModule):
    ...
    def forward(self, point):
        ...
        if _PTV3_ASSERT_INDICES:
            serial_n = int(point.serialized_order[self.order_index].shape[0])
            feat_n = int(point.feat.shape[0])
            extra = (
                f"order_index={self.order_index} patch_size={int(self.patch_size)} feat_n={feat_n} serial_n={serial_n} pad_n={int(pad.numel())} unpad_n={int(unpad.numel())}"
            )
            _assert_in_bounds("pad", pad, size=serial_n, extra=extra)

        order = point.serialized_order[self.order_index][pad]
        inverse = unpad[point.serialized_inverse[self.order_index]]

        if _PTV3_ASSERT_INDICES:
            _assert_in_bounds("order", order, size=feat_n, extra=extra)
            _assert_in_bounds("inverse", inverse, size=int(pad.numel()), extra=extra)

        # padding and reshape feat and batch for serialized point patch
        if self.add_coords_in_attn == "qkv":
            point.feat = point.feat + self.coords_proj(point.coord)
            qkv = self.qkv(point.feat)
            qkv = _safe_index(qkv, order, "attn_qkv_order")
        elif self.add_coords_in_attn == "qk":
            qkv = self.qkv(point.feat)
            qk_coords = self.coords_proj(point.coord).repeat(1, 2)
            qk_coords = torch.cat([qk_coords, torch.zeros(qkv.size(0), C, dtype=qkv.dtype).to(qkv.device)], dim=-1)
            qkv = qkv + qk_coords
            qkv = _safe_index(qkv, order, "attn_qkv_order")
        else:
            qkv = _safe_index(self.qkv(point.feat), order, "attn_qkv_order")
        ...
        feat = _safe_index(feat, inverse, "attn_feat_inverse")
...
class SerializedPooling(PointModule):
    ...
    def forward(self, point: Point):
        ...
        point_dict = Dict(
            feat=torch_scatter.segment_csr(_safe_index(self.proj(point.feat), indices, "pool_feat_indices"), idx_ptr, reduce=self.reduce),
            coord=torch_scatter.segment_csr(_safe_index(point.coord, indices, "pool_coord_indices"), idx_ptr, reduce="mean"),
    ...
    def forward(self, point):
        ...
        if _PTV3_ASSERT_INDICES:
            _assert_in_bounds(
                "pooling_inverse",
                inverse,
                size=int(point.feat.shape[0]),
                extra=f"unpool: parent_n={int(parent.feat.shape[0])} pooled_n={int(point.feat.shape[0])}",
            )

        parent.feat = parent.feat + _safe_index(point.feat, inverse, "unpool_feat_inverse")
